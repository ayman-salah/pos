...
import { interface_store } from 'src/app/intefaces/store';
//=========== end imports
@Component({
  selector: 'app-stores',
...
export class StoresComponent implements AfterViewInit {
  ...
  ngOnInit(): void {
    this.subscribe = this.generalService.getstores().subscribe((data) => {
      this.dataSource = new MatTableDataSource<interface_store>(data);
