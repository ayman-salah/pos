...
import { interface_suppliers } from 'src/app/intefaces/suppliers';
...
export class ListSuppliersComponent implements AfterViewInit {
  ...
  ngOnInit(): void {
    this.subscribe = this.generalService.get_suppliers().subscribe((data) => {
      this.dataSource = new MatTableDataSource<interface_suppliers>(data)
